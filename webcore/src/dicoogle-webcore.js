...
function check_initialized() {
  if (base_url === null) {
    console.error(
      "Dicoogle Webcore has not been initialized! Please call the init method."
    );
    return false;
  }
  return true;
...
export function init(baseURL) {
  ...
  Object.assign(Dicoogle, {
    ...
    emit,
    emitSlotSignal,
    addMenuPluginListener: addMenuPluginListener,
    addPluginLoadListener: addPluginLoadListener,
 ...
 * @return {void}
 */
export function removeEventListener(eventName, fn) {
  if (process.env.NODE_ENV !== "production" && !check_initialized()) return;
 ...
 * @return {void}
 */
export function addResultListener(fn) {
  if (process.env.NODE_ENV !== "production" && !check_initialized()) return;
 ...
 * @return {DicoogleWebcore} the webcore module itself, used for chaining
 */
export function addPluginLoadListener(fn) {
  if (process.env.NODE_ENV !== "production" && !check_initialized()) return;
 ...
 * @return {DicoogleWebcore} the webcore module itself, used for chaining
 */
export function addMenuPluginListener(fn) {
  if (process.env.NODE_ENV !== "production" && !check_initialized()) return;
...
export function updateSlots() {
  ...
  fetchPlugins(slotIds, function(packages) {
 ...
 * @return {void}
 */
export function updateSlot(elem, callback) {
  if (process.env.NODE_ENV !== "production" && !check_initialized()) return;
  if (typeof document !== "object") {
    throw "no DOM environment!";
  }
  ...
  if (pluginsOfSlot.length > 0) {
    ...
    attachAllPlugins(elem);
  } else {
    // fetch the needed plugins and load them
    fetchPlugins(elem.slotId, function(packages) {
      for (let i = 0; i < packages.length; i++) {
 ...
 * @return {void}
 */
export function fetchPlugins(slotIds, callback) {
  if (process.env.NODE_ENV !== "production" && !check_initialized()) return;
    ...
    for (let i = 0; i < packageArray.length; i++) {
      if (!packages[packageArray[i].name]) {
        ...
        if (packageArray[i].dicoogle.slotId === "menu") {
          event_hub.emit("menu", {
            name: packageArray[i].name,
            slotId: "menu",
            caption: packageArray[i].dicoogle.caption
          });
        }
      }
    }
 ...
 * @return {void}
 */
export function fetchModules(packages) {
  if (process.env.NODE_ENV !== "production" && !check_initialized()) return;
 ...
 * @return {void}
 */
export const onRegister = function(pluginInstance, name) {
 ...
 * @return {void}
 */
export function attachAllPlugins(elem) {
  if (process.env.NODE_ENV !== "production" && !check_initialized()) return;
...
// ----------------------------------------------------------------------------
export const WebUISlot = function(id, dom) {
  this.id = id;
  this.dom = dom;
  ...
  this.attachPlugin = function(plugin) {
    ...
    if (plugin.SlotId !== this.id) {
      console.error(
        `Attempt to attach plugin ${plugin.Name} to the wrong slot`
      );
      return;
    }
    if (
      typeof this.pluginName === "string" &&
...
function loadSlot(slotDOM) {
  ...
  slots[id].push(new WebUISlot(id, slotDOM));
...
function load_plugin(packageJSON, callback) {
  ...
  if (plugins[name]) {
    ...
    getScript(packageJSON.name, function() {
      ...
      if (!isFunction(constructors[name])) {
...
export class DicoogleSlotElement extends HTMLDivElement {
  constructor() {
    super();
  }
  ...
  connectedCallback() {
    ...
    if (base_url !== null) {
      updateSlot(this, pluginInstance => {});
    }
  }
...
customElements.define("dicoogle-slot", DicoogleSlotElement, { extends: "div" });
