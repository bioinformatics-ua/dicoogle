...
public class PluginController {
    ...
    private void initializePlugins(Collection<PluginSet> plugins) {
        for (PluginSet set : plugins) {
            ...
            final Collection<Collection<?>> all =
                    Arrays.asList(set.getStoragePlugins(), set.getIndexPlugins(), set.getQueryPlugins(), set.getJettyPlugins(), set.getRestPlugins());
    ...
    private void applySettings(PluginSet set, ConfigurationHolder holder) {
        // provide platform to each plugin interface
        final Collection<Collection<? extends DicooglePlugin>> all =
                Arrays.asList(set.getStoragePlugins(), set.getIndexPlugins(), set.getQueryPlugins(), set.getJettyPlugins());
    ...
    public List<String> filterDicomQueryProviders(List<String> providers) {
        ...
        return baseProviders.stream().filter(p -> providers.isEmpty() || providers.contains(p)).collect(Collectors.toList());
    }
    ...
    public JointQueryTask query(JointQueryTask holder, List<String> querySources, final String query, final DimLevel level, final Object... parameters) {
        if (holder == null)
            return null;
    ...
    private Task<Iterable<SearchResult>> getTaskForQueryDim(final String querySource, final String query, final DimLevel level, final Object... parameters) {
