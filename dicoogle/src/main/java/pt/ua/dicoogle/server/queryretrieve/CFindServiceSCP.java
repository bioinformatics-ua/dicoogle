...
public class CFindServiceSCP extends CFindService {
    ...
    /*** CFIND */
    @Override
    protected synchronized DimseRSP doCFind(Association as, int pcid, DicomObject cmd, DicomObject keys, DicomObject rsp) throws DicomServiceException {
        ...
        if (!superSpeed) {
            ...
            LogLine ll = new LogLine("cfind", getDateTime(), as.getCallingAET(), as.toString() + " -- " + add, queryParams);
