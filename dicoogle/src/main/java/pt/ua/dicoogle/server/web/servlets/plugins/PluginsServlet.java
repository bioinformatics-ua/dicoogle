...
public class PluginsServlet extends HttpServlet {
        ...
        try {
            ...
            if (all || "dead".equals(type)) {
                ...
                for (DeadPlugin p : pc.getDeadPluginSets()) {
                    writer.object().key("name").value(p.getName()).key("cause").object().key("class").value(p.getCause().getClass().getSimpleName())
                            .key("message").value(p.getCause().getMessage()).endObject().endObject();
                }
    ...
    private static JSONWriter writeBaseProps(JSONWriter writer, DicooglePlugin plugin, String type) throws JSONException {
        return writer.key("name").value(plugin.getName()).key("type").value(type).key("enabled").value(plugin.isEnabled());
    }
    ...
    private static JSONWriter writeStoragePlugin(JSONWriter writer, StorageInterface plugin) throws JSONException {
        return writeBaseProps(writer.object(), plugin, "storage").key("scheme").value(plugin.getScheme()).key("default").value(null).endObject();
    }
